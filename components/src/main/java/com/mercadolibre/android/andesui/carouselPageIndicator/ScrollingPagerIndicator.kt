...
class ScrollingPagerIndicator @JvmOverloads constructor(
    ...
    private fun initDots(itemCount: Int) {
        ...
        firstDotOffset = if (looped && this.itemCount > visibleDotCount) {
            0f
        } else {
            (dotSelectedSize / 2).toFloat()
        }
        visibleFrameWidth = ((visibleDotCount - 1) * spaceBetweenDotCenters + dotSelectedSize).toFloat()
        requestLayout()
        invalidate()
    }
