 ...
 */
sealed class AndesThumbnailBadgeComponent(internal val color: AndesBadgeIconType) {
    /**
    ...
    ) : AndesThumbnailBadgeComponent(color)
    ...
    private fun getAndesThumbnailBadgeType(): AndesThumbnailBadgeComponentInterface {
        return when (this) {
            is Pill -> AndesPillThumbnailBadgeComponent(text, textStyleDefault, color, thumbnailSize.size)
